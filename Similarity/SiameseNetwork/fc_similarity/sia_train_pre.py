...
import siamese_similarity as st
...
tf.flags.DEFINE_integer("embedding_type", 0, "1: trainable=true, 0: trainable=false")
tf.flags.DEFINE_string("embedding_file", hp.embedding_file, "pre-trained embedding")
tf.flags.DEFINE_boolean("use_position", True, "if use position embeddings")
...
def train():
    ...
    # 加载预训练词向量
    pretrained_emb, word2id, id2word = data_helper.read_vocab_emb(FLAGS.embedding_file, FLAGS.model_path)
        ...
        with sess.as_default():
            ...
            def train_step(batch_ids_a, batch_ids_b, batch_labels):
                ...
                _, loss, scores, preds, labels = sess.run(
                    [train_op, model.loss, model.softmax_scores, model.predictions, model.labels], feed_dict=feed_dict
                )
                print "actucal-{} predict-{}".format(labels[:32], preds[:32])
                tp, fp, tn, fn = get_accu(scores[:, 1], batch_labels[:, 1])
            ...
            def validation_step(batch_ids_a, batch_ids_b, batch_labels):
                ...
                loss, scores = sess.run(
                    [model.softmax_score_losses, model.softmax_scores],
                    feed_dict=feed_dict
                )
                tp, fp, tn, fn = get_accu(scores[:, 1], batch_labels[:, 1])
                return loss, tp, fp, tn, fn
